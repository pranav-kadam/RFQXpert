...
async def upload_file(file: UploadFile = File(...)):
    ...
    chunks = load_user_file(io.BytesIO(file_data), file.filename)
    if not chunks:
        return {"error": "Failed to process file."}
    
    stored = store_embeddings(chunks, file.filename)
    return {"message": f"Stored {len(stored)} embeddings.", "filename": file.filename}

@app.post("/query")
async def query_text(payload: Dict[str, str]):
    ...
    prompt = f"""You are an assistant helping answer questions based on context.
    Context:\n{combined_context}\n
    Question: {query}
    Answer:"""
